...
interface SocketEvents {
  ...
  onAnnotationResolved: (data: { annotationId: string; resolvedBy: string; timestamp: string }) => void
  onAnnotationReplyAdded: (data: { projectId: string; annotationId: string; reply: AnnotationReply; timestamp: string }) => void
  onAnnotationStatusUpdated: (data: { annotationId: string; projectId: string; status: string; isResolved: boolean; timestamp: string }) => void
  onTyping: (data: { projectId: string; user: string; isTyping: boolean; timestamp: string }) => void
}

export function useRealtimeAnnotations(projectId: string, events: SocketEvents) {
  ...
  const resolveAnnotation = useCallback((annotationId: string, resolvedBy: string) => {
    if (!socket || !isConnected) return
  ...
  const addAnnotationReply = useCallback((replyData: {
    annotationId: string
    content: string
    addedBy: string
    addedByName: string
  ...
  const updateAnnotationStatus = useCallback((annotationId: string, status: string, resolvedBy?: string) => {
    if (!socket || !isConnected) return
  ...
  const sendTypingIndicator = useCallback((isTyping: boolean, user: string) => {
    if (!socket || !isConnected) return
